...
// קומפוננטה לרינדור חכם של נתונים וטבלאות - כולל הגנה מריסוקים
      ...
      <div className="my-4 rounded-2xl border border-sky-200/30 bg-slate-900/95 p-4 shadow-2xl overflow-x-auto ring-1 ring-white/10">
        <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-mono text-slate-400 uppercase tracking-widest font-black tracking-widest">SabanOS Engine Output</span>
          </div>
    ...
    <div className="fixed inset-0 h-screen w-screen bg-[#f1f5f9] flex flex-col md:flex-row overflow-hidden font-assistant" dir="rtl">
      ...
      <div className="hidden md:flex w-80 bg-white/40 backdrop-blur-3xl border-l border-white/20 flex-col p-8 shrink-0 z-50 shadow-2xl relative">
        ...
        <div className="flex items-center gap-4 mb-12 relative z-10">
          ...
          <h1 className="text-2xl font-black text-slate-900 tracking-tighter italic">Noa <span className="text-sky-500">Pro</span></h1>
        </div>
        ...
        <div className="space-y-6 relative z-10">
          ...
          <div className="p-6 rounded-[2.5rem] bg-slate-900 text-white shadow-2xl border border-white/5 relative overflow-hidden group">
            ...
            <p className="text-[11px] leading-relaxed text-slate-400 font-medium relative z-10">
              החיבור כעת ישיר ל-Google AI ללא צורך בפרוקסי של ורסל.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col h-full relative bg-white overflow-hidden">
        {/* Diamond Polishing Gloss Overlays */}
          ...
          <AnimatePresence>
            {chatHistory.length === 0 && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center justify-center min-h-[60vh] text-center">
                <div className="w-24 h-24 bg-slate-900 rounded-[3.5rem] flex items-center justify-center shadow-2xl mb-8 relative">
                ...
                <h2 className="text-4xl font-black text-slate-900 mb-6 italic tracking-tight">איך נתקדם היום, ראמי?</h2>
                <div className="flex flex-wrap justify-center gap-3 max-w-lg px-4">
                  {suggestions.map(s => (
                    <button 
                      key={s.label}
                      onClick={() => onAction(s.action)}
                      className="px-6 py-4 bg-white shadow-xl shadow-slate-200/50 rounded-[2.5rem] border border-slate-50 text-sm font-bold text-slate-700 hover:bg-slate-900 hover:text-white transition-all active:scale-95"
                    >
                      {s.label}
                    </button>
                  ))}
            ...
            {isLoading && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex justify-end w-full">
                <div className="bg-white/95 p-6 rounded-[2.5rem] rounded-tl-none shadow-xl border border-sky-100 flex items-center gap-4">
                  ...
                  <span className="text-xs font-black text-sky-600 italic tracking-tighter uppercase tracking-widest">Noa Thinking...</span>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-white via-white to-transparent pt-12 pb-10 px-6 z-40">
          <div className="max-w-4xl mx-auto relative group">
             <div className="absolute -inset-1 bg-gradient-to-r from-sky-400 to-blue-600 rounded-[3.5rem] blur opacity-10 group-focus-within:opacity-25 transition duration-1000" />
             <form onSubmit={handleSubmit} className="relative flex items-center">
              <input 
                value={inputValue}
                onChange={(e) => setInputValue(e.target.value)}
                placeholder="כתוב לנועה... (מלאי, נהגים, הזמנות)"
                className="w-full bg-white border-2 border-slate-100 rounded-[3rem] pl-24 pr-10 py-6 md:py-8 text-lg font-bold focus:border-sky-600 transition-all outline-none shadow-2xl placeholder:text-slate-300"
              />
              <button 
                type="submit"
                disabled={isLoading}
                className="absolute left-4 p-5 md:p-6 bg-slate-900 text-white rounded-full hover:bg-sky-600 transition-all shadow-xl active:scale-90 z-20 disabled:opacity-50"
              >
