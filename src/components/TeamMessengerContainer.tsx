...
export const TeamMessengerContainer: React.FC<TeamMessengerContainerProps> = ({ currentUserProfile }) => {
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="mb-4 w-[350px] sm:w-[800px] h-[600px] bg-white/80 backdrop-blur-3xl rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/20 flex flex-col sm:flex-row"
          >
            {/* Sidebar - Desktop Only Split View */}
            <div className={`w-full sm:w-1/3 bg-gray-50/50 border-l border-gray-200/50 flex flex-col ${selectedMember ? 'hidden sm:flex' : 'flex'}`}>
              ...
              <div className="flex-1 overflow-y-auto p-2">
                {teamMembers.map(member => (
                   ...
                   >
                     <div className="relative">
                        <img src={member.avatarUrl} className="w-12 h-12 rounded-xl object-cover border-2 border-white" alt="" />
            ...
            {/* Main Chat Area */}
            <div className={`flex-1 flex flex-col bg-white/30 ${selectedMember ? 'flex' : 'hidden sm:flex'}`}>
              {!selectedMember && (
              ...
              {selectedMember && (
                <>
                  <div className="p-6 bg-white/50 border-b border-gray-200/50 flex justify-between items-center backdrop-blur-md">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="relative">
                        <img src={selectedMember.avatarUrl} className="w-10 h-10 rounded-xl object-cover" alt="" />
                      </div>
                      <div>
                        <h4 className="font-black text-gray-900 leading-none">{selectedMember.name}</h4>
                        <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">
                          {isOnline(selectedMember.lastSeen) ? 'פעיל כעת' : 'לא מחובר'}
                        </span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                       ...
                       <button onClick={() => setIsOpen(false)} className="p-2.5 hover:bg-red-50 text-gray-400 hover:text-red-600 rounded-xl sm:hidden"><X size={18}/></button>
                    </div>
                  </div>

                  <div 
                    ref={scrollRef}
                    className="flex-1 overflow-y-auto p-6 space-y-2 bg-gradient-to-b from-transparent to-sky-50/20"
                  >
                    {messages.map((msg, i) => (
                       <MessageBubble 
                         key={msg.id || i}
                  ...
                  <form onSubmit={handleSendMessage} className="p-6 bg-white/50 border-t border-gray-200/50 flex flex-col gap-4">
                    <div className="flex items-center gap-2">
                       <div className="relative flex-1">
                          <input 
                            ...
                            className="w-full bg-white border border-gray-100 rounded-2xl p-4 pr-12 text-sm font-bold shadow-sm outline-none focus:ring-2 focus:ring-sky-600/10 transition-all"
                          />
                          <button 
                            type="button"
                       ...
                       <button 
                         type="submit"
                         className="bg-sky-600 text-white p-4 rounded-2xl hover:bg-sky-700 transition-all shadow-xl shadow-sky-600/20"
                       >
                         <Send size={24} />
                       </button>
                    </div>
                    ...
                    <AnimatePresence>
                        {showMentions && (
                            <motion.div 
                                ...
                                exit={{ opacity: 0, y: 10 }}
                                className="absolute bottom-32 right-6 w-64 bg-white rounded-2xl shadow-2xl border border-gray-100 p-2 overflow-hidden"
                            >
                                ...
                                <div className="max-h-40 overflow-y-auto">
                                    {teamMembers.map(m => (
                                        ...
                                        >
                                            <img src={m.avatarUrl} className="w-6 h-6 rounded-lg object-cover" alt="" />
