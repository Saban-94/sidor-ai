...
Scan input for `[DEVICE: MOBILE]` or `[DEVICE: DESKTOP]` tags:
- **📱 MOBILE**: 1 column layout, compact padding (`p-2`), buttons min-height `48px` for touch.
- **🖥️ DESKTOP**: Multi-column grid (`grid-cols-2` or `3`), spread layout (`p-4`), rich data display.

## Interactive Buttons (Tactical)
Render `<button>` with `data-intent` and `data-payload`:
